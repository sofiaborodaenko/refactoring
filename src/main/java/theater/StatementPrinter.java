...
public class StatementPrinter {
    ...
    private final int fortyThousand = 40000;
    ...
    public String statement() {
        ...
        for (Performance performance : invoice.getPerformances()) {
            ...
            result.append(String.format("  %s: %s (%s seats)%n", getPlay(performance).getName(),
                    frmt.format(getAmount(performance) / oneHundred), performance.getAudience()));
            totalAmount += getAmount(performance);
        }
        result.append(String.format("Amount owed is %s%n", frmt.format(totalAmount / oneHundred)));
        ...
        switch (getPlay(performance).getType()) {
            case "tragedy":
                ...
                if (performance.getAudience() > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                    thisAmount += oneThousand * (performance.getAudience() - thirty);
                }
                break;
            case "comedy":
